...
type StreamST struct {
	...
	ViewerCheckInterval int    `json:"viewer_check_interval"` // Interval in seconds to check for viewers (default: 20)
	RunLock             bool   `json:"-"`
	Codecs              []av.CodecData
...
func (element *ConfigST) RunIFNotRun(uuid string) {
	...
	if tmp, ok := element.Streams[uuid]; ok {
		if tmp.OnDemand && !tmp.RunLock {
			...
			go RTSPWorkerLoop(uuid, tmp.URL, tmp.OnDemand, tmp.DisableAudio, tmp.Debug, tmp.ViewerCheckInterval)
		}
	}
}
